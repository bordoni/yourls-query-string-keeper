...
Plugin URI: https://github.com/bordoni/yourls-query-string-keeper
Description: Adds short URL query string, if any, to the long URL
Version: 1.0
Author: Ozh, Rich Christiansen, Bordoni
*/

/**
 * This was forked from https://github.com/rinogo/yourls-keep-query-string
 */

yourls_add_filter( 'redirect_location', 'tec_kqs' );

function tec_kqs( $url ) {
	if ( yourls_is_GO() ) {
		//$url = yourls_add_query_arg($_GET, $url);
		$url = tec_kqs_yourls_add_query_arg( $_GET, $url );
	}
 ...
 */
function tec_kqs_yourls_add_query_arg() {
	$ret = '';
